...
void PlayerbotFactory::Randomize(bool incremental)
{
    ...
    // Strict BiS wins over equipment persistence: persistence exists to preserve gear a bot
    // earned/was given, which is exactly what strict mode is meant to overrule. When it does
    // not apply, the normal persistence rule decides.
    if (!ApplyStrictBisEquipment(bot) &&
        (!incremental || !sPlayerbotAIConfig.equipAndSpecPersistence ||
         bot->GetLevel() < sPlayerbotAIConfig.equipAndSpecPersistenceLevel))
    {
